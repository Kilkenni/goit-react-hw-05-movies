...
class TmdbMovieGetSubdata extends TmdbGetMethod { //Abstract!
    ...
    constructor(movie_id, language = "") {
        super();
        if (this.constructor === TmdbMovieGetSubdata) {
            throw new Error("TmdbMovieGetSubdata class is abstract. Abstract classes can't be instantiated.");
        }     
    ...
    toString() {
        ...
        return URL_with_params.replace('?', this.movie_id + this.TMDB_API_ENDPOINT + '?');
    }
}

class TmdbMovieData extends TmdbMovieGetSubdata {
    constructor(movie_id, language = "") {
...
class TmdbMovieCredits extends TmdbMovieGetSubdata {
    TMDB_API_ENDPOINT = "/credits";
...
Its primary function is to generate correct URLs for TheMovieDatabase API. */
...
export class TmdbApiService {
    ...
    constructor(handler, handlerParameters = {}) {
        ...
        switch (handler) {
            ...
            case "TMDB_movieCredits": {
                const { movie_id, language } = handlerParameters;
                this.handler = new TmdbMovieCredits(movie_id, language);
                break;
            }
